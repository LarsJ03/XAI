...
const excludedMethods = new Set(["alibi-CF"]);
...
export const methods = [
  {
    ...
    label: "PIECE",
    description:
      "PIECE generates counterfactuals for images by changing semantically meaningful superpixels. It works in two stages. First, the image is divided into multiple superpixels, and each superpixel is assigned a probability of being relevant to the classification. Next, these superpixels are replaced according to their assigned probabilities while searching for a combination that changes the predicted class. Because changes are localized to coherent image regions, the resulting counterfactuals tend to be interpretable."
  },
  {
    id: "Min-Edit",
    label: "Min-Edit",
    description:
      "Min-Edit focuses on finding the minimal amount of change to the input that causes a class flip. Its optimization objective is to minimize the number of changed superpixels. Since this method looks for the minimal amount of change, the counterfactuals might not be visually coherent. This can be a negative property because these tools are designed for interpretation purposes."
  },
  {
    id: "C-Min-Edit",
    label: "C-Min-Edit",
    description:
      "C-Min-Edit is very similar to Min-Edit, with the C standing for constrained. It extends Min-Edit by adding a realism constraint on the changes, restricting superpixel changes to those that are plausible given the data distribution. This method focuses on the balance between minimality and plausibility. It directly addresses a problem of Min-Edit by aiming for counterfactuals that are both simple and visually coherent. However, this constraint also narrows the search space and can cause the method to fail in some cases."
  },
  {
    id: "alibi-Proto-CF",
    label: "Alibi Proto-CF",
    description:
      "Alibi Proto-CF searches in the feature space toward the nearest prototype of the target class. First, a set of prototypes is precomputed using k-means or an autoencoder. A prototype is a representative embedding for a class. Next, an optimization algorithm moves the input in the feature space until it is classified as the target class. The loss function balances prediction loss, proximity to the original image, and proximity to the prototype. Prediction loss penalizes images that are not classified as the target class, while proximity to the original image and prototype supports realism and coherence."
  }
];
...
function hydrateExamples(examples) {
  return examples.map((trial) => ({
    ...trial,
    originalAsset: withPublicAsset(trial.originalAsset),
    options: trial.options
      .filter((option) => option.available && !excludedMethods.has(option.method))
      .map((option) => ({
        ...option,
...
export const prepExamples = hydrateExamples(sourceData.prepExamples || []);
export const trialExamples = hydrateExamples(sourceData.trials.slice(0, 5));
